...
angular.module('sbAdminApp')
  .controller('userManagementCtrl', function($scope,$position,$http,$modal) {
    var vm = this;
    vm.detailTab=false;
  ...
  $scope.totalItems = 0;
  vm.users2=[];
    vm.getusers=function(currentPage){
         $http.get('/api/getusers?pageNo='+currentPage)
        .then(function(data) {
            if (data) {
                console.log(data);
                ...
                $scope.exportData = [];
                $scope.exportData.push(["#", "firstName", "email"]);
                angular.forEach(vm.users2, function(value, key) {
                $scope.exportData.push([value._id, value.firstName, value.email]);
                });
            }else{
